  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        <section className="welcome-row"><div><p className="eyebrow">MONDAY, SEPTEMBER 07, 2026</p><h1>Good morning <span>✦</span></h1><p className="lede">A clear view of your health, with context you can understand.</p></div><button className="primary-button" onClick={() => setShowUpload(true)}><Upload size={17} /> Upload a report</button></section>
    ...
    {showUpload && <div className="modal-backdrop" onClick={() => setShowUpload(false)}><div className="upload-modal" role="dialog" aria-modal="true" aria-labelledby="upload-title" onClick={(event) => event.stopPropagation()}><button className="close-button" aria-label="Close upload dialog" onClick={() => setShowUpload(false)}>×</button><div className="upload-symbol"><Upload size={24} /></div><h2 id="upload-title">Upload a health report</h2><p>Drop a PDF or image here to extract values and compare them with the lab's own reference ranges.</p><label className="drop-zone"><input type="file" accept=".pdf,image/*" onChange={handleFileChange} /><FileText size={24} /><strong>{selectedFile ? selectedFile.name : 'Choose a file'}</strong><span>{uploadError || 'PDF, JPG, or PNG up to 20 MB'}</span></label><small className="modal-note"><ShieldCheck size={14} /> Demo mode: files are not sent anywhere yet.</small></div></div>}
  </div>
}
export default App
